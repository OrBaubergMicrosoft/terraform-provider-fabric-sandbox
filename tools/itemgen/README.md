...
1. **Basic** (`basic`)
   - No definition, no properties, no config — the simplest item type
   - Example: `mlmodel`, `mlexperiment`, `graphqlapi`
   - Uses: `NewDataSourceFabricItem`, `NewDataSourceFabricItems`, `NewResourceFabricItem`
   - Note: Basic items use the generic `fabcore.Item` fake handler, so no typed fake handler is generated
...
1. Generate a basic item (like mlmodel):

   ```bash
   go run tools/itemgen/main.go \
   -item-name="ML Model" \
   -items-name="ML Models" \
   -item-type=basic \
   -rename-allowed=false \
   -is-preview=true \
   -is-spn-supported=false
   ```
