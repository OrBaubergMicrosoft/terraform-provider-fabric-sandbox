...
var _ datasource.DataSourceWithConfigure = (*dataSourceEventstreamDestinationConnection)(nil)

type dataSourceEventstreamDestinationConnection struct {
	pConfigData *pconfig.ProviderData
	client      *fabeventstream.TopologyClient
	TypeInfo    tftypeinfo.TFTypeInfo
}

func NewDataSourceEventstreamDestinationConnection() datasource.DataSource {
	return &dataSourceEventstreamDestinationConnection{
		TypeInfo: ItemTypeInfo,
	}
}

func (d *dataSourceEventstreamDestinationConnection) Metadata(_ context.Context, _ datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = d.TypeInfo.FullTypeName(false)
}

func (d *dataSourceEventstreamDestinationConnection) Schema(ctx context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *dataSourceEventstreamDestinationConnection) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *dataSourceEventstreamDestinationConnection) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "READ", map[string]any{
		"action": "start",
	})

	var data dataSourceEventstreamDestinationConnectionModel
...
func (d *dataSourceEventstreamDestinationConnection) get(ctx context.Context, model *dataSourceEventstreamDestinationConnectionModel) diag.Diagnostics {
	respGet, err := d.client.GetEventstreamDestinationConnection(ctx, model.WorkspaceID.ValueString(), model.EventstreamID.ValueString(), model.DestinationID.ValueString(), nil)
